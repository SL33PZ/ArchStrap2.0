...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        if os.path.isfile("tmp/.hashed"):
            os.remove("tmp/.hashed")
...
"")
        ...
        self.checkBox_Onedrive = QtWidgets.QCheckBox(parent=self.groupBox_optionals)
        font = QtGui.QFont()
        font.setFamily("Comfortaa")
        font.setPointSize(11)
        font.setBold(False)
        ...
        font.setWeight(50)
        font.setStyleStrategy(QtGui.QFont.StyleStrategy.PreferAntialias)
        self.checkBox_Onedrive.setFont(font)
        self.checkBox_Onedrive.setObjectName("checkBox_Onedrive")
        self.gridLayout.addWidget(self.checkBox_Onedrive, 2, 2, 1, 1)
        self.checkBox_ipe = QtWidgets.QCheckBox(parent=self.groupBox_optionals)
        font = QtGui.QFont()
        font.setFamily("Comfortaa")
        font.setPointSize(11)
        font.setBold(False)
        ...
        font.setWeight(50)
        font.setStyleStrategy(QtGui.QFont.StyleStrategy.PreferAntialias)
        self.checkBox_ipe.setFont(font)
        self.checkBox_ipe.setObjectName("checkBox_ipe")
        self.gridLayout.addWidget(self.checkBox_ipe, 2, 1, 1, 1)
        ...
        self.checkBox_chrome = QtWidgets.QCheckBox(parent=self.groupBox_optionals)
        font = QtGui.QFont()
        font.setFamily("Comfortaa")
        font.setPointSize(11)
        font.setBold(False)
        ...
        self.gridLayout.addWidget(self.checkBox_chrome, 1, 0, 1, 1)
...
" }")
        ...
        MainWindow.setTabOrder(self.checkBox_npm, self.checkBox_chrome)
        MainWindow.setTabOrder(self.checkBox_chrome, self.checkBox_vscode)
        ...
        MainWindow.setTabOrder(self.checkBox_klassy, self.checkBox_ipe)
        MainWindow.setTabOrder(self.checkBox_ipe, self.checkBox_Onedrive)
        MainWindow.setTabOrder(self.checkBox_Onedrive, self.checkBox_inkscape)
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.checkBox_ipe.setText(_translate("MainWindow", "Ipe"))
...
# ××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××× #
        ...
        self.comboBox_efi.currentTextChanged.connect(self.validate_efi)
        self.comboBox_swap.currentTextChanged.connect(self.validate_swap)
        self.comboBox_root.currentTextChanged.connect(self.validate_root)
...
# ××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××××× #
    ...
    def control_instance(self):
        ...
        if all(controlles):
            ...
            env_data = [
                ...
                ("EFI", "/dev/" + self.comboBox_efi.currentText()),
                ("SWAP", "/dev/" + self.comboBox_swap.currentText()),
                ("ROOT", "/dev/" + self.comboBox_root.currentText()),
            ]   
            ...
            with open("tmp/.env", "a") as env:
                ...
                for item, name in zip(self.checkBoxes, self.checkBoxes_names.values()):
                    if item.isChecked():
                        opts.append(name)
                ...
                base_packages=['dosfstools', 'ntfs-3g', 'parted', 'gdisk', 'cmake', 'extra-cmake-tools', 'git']
                env_packages=['base','base-devel', 'linux-firmware', 'linux-headers', 
                              'xorg', 'xorg-server', 'xorg-xinit', 'networkmanager', 
                              'network-manager-applet', 'git', 'wget', 'curl', 'aria2', 
                              'axel', 'lftp', 'nano', ' cmake', 'extra-cmake-tools', 
                              'dosfstools', 'mtools', 'efibootmgr', 'pipewire', 'alsa-utils', 
                              'alsa-plugins', 'alsa-firmware', 'xf86-video-intel', 
                              'intel-media-driver', 'intel-ucode', 'mesa', 'vulkan-intel', 
                              'pacman-contrib', 'pac-uitls', 'python-pip', 'perl', 'ruby', 
                              'go', 'python-gobject']
                                
                opts=str(opts).replace(',', '').replace('[', '(').replace(']', ')')
                base_p=str(base_packages).replace(',', '').replace('[', '(').replace(']', ')')
                env_p=str(env_packages).replace(',', '').replace('[', '(').replace(']', ')')
                
                env.write(f"export BASE_PACKAGES={base_p}\n")
                env.write(f"export ENV_PACKAGES={env_p}\n")
                
                if opts != "":
                    env.write(f"export OPTS={opts}\n")
                
            self.close()
            
            rndm_pw=secrets.token_urlsafe(32)


            subprocess.run(["bash", "lib/encode.sh", f"{self.lineEdit_userPw.text()}", f"{rndm_pw}", f"{self.lineEdit_rootPw.text()}"], shell=False)
    ...
    def validate_efi(self):
        self.efi = self.comboBox_efi.currentIndex() != -1
        
    def validate_swap(self):
        self.swap = self.comboBox_swap.currentIndex() != -1
        
    def validate_root(self):
        self.root = self.comboBox_root.currentIndex() != -1
